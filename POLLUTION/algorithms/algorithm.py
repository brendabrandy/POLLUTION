...
#change NaN, US , keshoka county and 1020 to others
#declare unique country names
df['source_O3'] = ""
df['source_NO2'] = ""
df['source_SO2'] = ""
df['source_CO'] = ""
...
def matrix(df,year,month,pollutant):
    ...
    for b in range(0,total_rows):
        ...
        for item in grouppy_dict:
            if (grouppy_dict[item] < 0.01):
                item_del.append(item)
        for deletes in item_del:
            del grouppy_dict[deletes]
        #sort the dictionary by state symbol
        #if the number of symbols is less than 50
        #cross check with state symbol list
        #if one does not match the other
        # add a tag to that particular location
